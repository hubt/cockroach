...
import (
	...
	"github.com/cockroachdb/cockroach/server/stats"
...
func newStatsServer(kvDB kv.DB) *statsServer {
	return &statsServer{
		kvDB: kvDB,
	}
}
...
func (s *statsServer) handleStats(w http.ResponseWriter, r *http.Request) {
	...
	kv.GetI(s.kvDB, storage.MakeKey(storage.KeyConfigAccountingPrefix, storage.KeyMin), acctConfig)

	c := &stats.Cluster{
		ID: acctConfig.ClusterID,
	}
